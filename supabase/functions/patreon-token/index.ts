...
Deno.serve(async (req) => {
  ...
  if (!clientId || !clientSecret) {
    return new Response(
      JSON.stringify({ error: "Server misconfigured: missing Patreon credentials" }),
  ...
  // Patreon requires form-urlencoded
  ...
  const upstreamResponse = await fetch("https://www.patreon.com/api/oauth2/token", {
    method: "POST",
